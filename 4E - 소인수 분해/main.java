...
class MathUtil{
	...
	public static ArrayList<Long> factorize(long N)
	{
		ArrayList<Long> factors = new ArrayList<>();
			...
			// 소수는 약수를 가지고 있지 않기 때문에 sqrt(N) 이하의 범위만 순회하고 약수를 발견하지 못했다면 탐색을 종료한다.
		...
		// 위 반복문 때문에 N은 마지막으로 탐색했던 sqrt(i)의 범위 보다 작은 소수임이 증명되었다.
		// 그 수가 딱 나누어 떨어져 1이되지 않고 1보다 큰 수라면 그 수는 N의 가장 큰 소인수임이 자명하다.
		if (N > 1) {
			factors.add(N);
		}
